...
def recebe_dados(client, tam_pacote, info, server_ip, server_port):
    print(f"Abrindo arquivo '{info}' e preparando para receber os pacotes...")
    f = open(info, 'wb')
...
if len(sys.argv) < 4:
    ...
    print(f"Uso: python3 {sys.argv[0]} <metrica> <tam_pacote> <nome_arquivo> <num_bytes> <host> <server_port>")
    sys.exit(1)
